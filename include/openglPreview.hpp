...
class OpenGLPreview {
  public:
    OpenGLPreview(Scene *_scene) : camera(_scene->VP, glm::normalize(_scene->LA - _scene->VP)) {
        glfwInit();
        ...
        window = glfwCreateWindow(((double)scene->xres / scene->yres) * previewHeight, previewHeight, "Chiaroscuro",
                                  NULL, NULL);
        if (window == NULL) {
    ...
    void setRenderer(RayCaster *renderer) { screen.renderer = renderer; }
  ...
  private:
    struct Screen {
        ...
        Screen(int texWidth, int texHeight)
            : width(texWidth), height(texHeight), shader("shader/screen.vs", "shader/screen.fs") {
    ...
    void setCallbacks() {
        ...
        glfwSetCursorPosCallback(window, [](GLFWwindow *window, double xpos, double ypos) {
            ...
            static float lastX = 0.5f * previewHeight;
            static float lastY = 0.5f * previewHeight;
